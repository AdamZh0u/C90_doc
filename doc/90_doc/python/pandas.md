...
mode="w") as writer:  #Append or Write
...
# Tricks
* Isnull 与 == None
	* [As the comment above states, missing data in `pandas` is represented by a _NaN_, where _NaN_ is a numerical value, i.e float type. However _None_ is a Python `NoneType`, so _NaN_ will not be equivalent to _None_.](https://stackoverflow.com/questions/58190953/why-does-pandas-isnull-work-but-none-not-work/58203510)
* fillna 用另一列填充
```python
df_street_built_up[('街道编号及名称','街道编号')].fillna(pd.Series(df_street_built_up.index),inplace=True)
df_street_built_up[('街道编号及名称','街道名称')].fillna(df_street_built_up[('街道编号及名称','街道')],inplace=True)
```
